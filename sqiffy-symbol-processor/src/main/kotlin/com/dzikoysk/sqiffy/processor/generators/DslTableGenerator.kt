...
class DslTableGenerator(private val context: KspContext) {
    ...
    internal fun generateTableClass(parsedDefinition: ParsedDefinition, tableName: String, properties: List<PropertyData>) {
        val infrastructurePackage = parsedDefinition.getInfrastructurePackage()
        val autogeneratedKey = properties.firstOrNull { it.type == DataType.SERIAL || it.type == DataType.BIGSERIAL }
                ...
                TypeSpec.objectBuilder(objectName)
                    .superclass(
                        when {
                            autogeneratedKey != null ->
                                TableWithAutogeneratedKey::class.asTypeName().parameterizedBy(
                                    (autogeneratedKey.mappedTo ?: autogeneratedKey.type!!.mappedTo.toTypeDefinition()).let {
                                         ClassName(it.packageName ?: "", it.simpleName)
                                    }
                                )
                            else ->
                                Table::class.asTypeName()
                        }
                    )
                    .addSuperclassConstructorParameter(CodeBlock.of(q(tableName)))
                    .also {
                        if (autogeneratedKey != null) {
                            it.addFunction(
                                FunSpec.builder("getAutogeneratedKey")
                                    .addModifiers(KModifier.OVERRIDE)
                                    .addStatement("""return ${autogeneratedKey.formattedName}""")
                    ...
                    .also { typeBuilder ->
                        properties.forEach {
                            ...
                            val property = PropertySpec.builder(it.formattedName, propertyType)
    ...
    private fun generateInsertValues(
        ...
        val insertionProperties = properties.filter { it.type != DataType.SERIAL && it.type != DataType.BIGSERIAL }
        val autogeneratedKey = properties.firstOrNull { it.type == DataType.SERIAL || it.type == DataType.BIGSERIAL }
                ...
                .addFunction(
                    FunSpec.builder("values")
                        .also { valuesBuilder ->
                            insertionProperties.forEach { property ->
                                valuesBuilder.addParameter(ParameterSpec
                                    .builder(property.formattedName, property.type!!.toTypeName(property))
                        ...
                        .addStatement(
                            ...
                            insertionProperties.joinToString("") { "it[${objectName}.${it.formattedName}] = ${it.formattedName};\n" }
                        )
                    ...
                    insertTypeBuilder.addFunction(
                        FunSpec.builder("values")
                            ...
                            .addCode(
                                ...
                                """.trimMargin(),
                                ClassName(infrastructurePackage, objectName),
                                insertionProperties.joinToString(separator = ";\n  ") { "it[${objectName}.${it.formattedName}] = entity.${it.formattedName}" }
                            )
                            .returns(insertStatementReturnType)
                            .build()
                    )
    ...
    private fun generateColumnInitializer(property: PropertyData): String {
        var baseColumn = with(property) {
            ...
            if (property.mappedTo != null) {
                ...
                DataType.BIGSERIAL -> "bigserial(${q(name)}, $defaultDbType)"
